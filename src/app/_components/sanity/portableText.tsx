...
const PortableTextComponents = {
  ...
  marks: {
    link: ({ children, value }: any) => {
      ...
      return (
        <a className='flex gap-1 items-center text-cyan-400/80  max-w-lg hover:text-cyan-400 z-50 text-sm md:text-base transition-all duration-500 group' href={value.href} rel={rel}>
  ...
  block:{
    blockquote: ({children}:any) => <blockquote className="border-l-8 px-1 border border-gray-800/50 border-l-gray-600/50 bg-black/20  z-0 text-sm md:text-base backdrop-blur-md  text-gray-400 md:p-4 my-2">{children}</blockquote>,
