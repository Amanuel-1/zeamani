...
const ShareBar = ({post}:shareBarProps) => {
  return (
    <ul className="social-card  flex flex-row justify-center items-center w-full lg:max-w-fit lg:flex-col gap-5 px-4 py-8 bg-transparent lg:bg-stone-900 rounded-r-[10px] text-stone-100">
        <li className="text-2xl text-center hover:scale-105 hover:text-amber-600 cursor-pointer transition-all duration-700"><FacebookShareButton url={`https://amanapps.vercel.app/${post.slug.current}`} hashtag={post.categories[0]?.title}><FaFacebookF/></FacebookShareButton></li>
